...
try:
    nltk.data.find("facebook/bart-large-cnn")
except LookupError:
    nltk.download("punkt")
...
def main():
    ...
    tokenized_train_dataset = train_dataset.map(
        lambda batch: tokenize_dataset(batch, bart_tokenizer), 
        batched=True
    )
    ...
    bart_model = get_peft_model(bart_model, lora_config)
    print(bart_model.print_trainable_parameters())
    ...
    data_collator = DataCollatorForSeq2Seq(bart_tokenizer, model = bart_model, return_tensors="pt")

    training_args = TrainingArguments(
        output_dir='./bart-thourght-lora',
